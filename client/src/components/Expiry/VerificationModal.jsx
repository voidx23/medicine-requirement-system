        ...
        <Modal isOpen={isOpen} onClose={onClose} title={`Verify Box: ${expiryList.branchId?.name}`}>
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: '600px' }}>
                ...
                <div style={{ background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px', gap: '1rem', padding: '0.75rem 1rem', background: '#f1f5f9', fontWeight: 600, fontSize: '0.85rem', color: '#475569' }}>
                        <div>Medicine</div>
                    ...
                    <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
                        {items.map((item, idx) => (
                            <div key={idx} style={{ 
                                display: 'grid', gridTemplateColumns: '1fr 80px 100px 100px', gap: '1rem', 
                                padding: '0.75rem 1rem', alignItems: 'center', 
                                borderBottom: idx !== items.length - 1 ? '1px solid #e2e8f0' : 'none',
                                background: item.qtyReceived !== item.qtySent ? 'rgba(239, 68, 68, 0.05)' : 'transparent'
                            }}>
                                <div>
                                ...
                                <div style={{ textAlign: 'center', fontWeight: 600 }}>{item.qtySent}</div>
